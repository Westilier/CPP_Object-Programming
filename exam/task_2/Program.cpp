	...
	while (true) {
		if (m_activeUser.IsAdmin())
		{
			std::cout << "1 - изменить пароль" << std::endl
				...
				<< "5 - модификацировать ползователя" << std::endl
				<< "6 - просмотреть статистику" << std::endl
				<< "7 - добавлить категорию" << std::endl
				<< "8 - добавлить тест" << std::endl
				<< "9 - добавить вопрос к тесту" << std::endl
				<< "10 - задать правильный ответ" << std::endl
				<< "11 - импортировать категории из папки" << std::endl
